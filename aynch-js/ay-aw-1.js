...
// -----------Following results are achived with Async Await-----------
async function showData() {
  try {
    var userDetails = await fetchUser(5);
    var watchedMovies = await fetchWatchedMovies(userDetails.id);
    var recommendations = await fetchRecommendations(watchedMovies);
    console.log(`User: ${userDetails.name}`);
    console.log(`Watched: ${watchedMovies.join(", ")}`);
...
// -----------Following results are achived with Promises-----------
// fetchUser(5)
//   .then(function (user) {
